...
def show_image_with_boxes(img, objects, calib, show3d=True):
    ...
    for obj in objects:
        ...
        # if abs(obj.ymax - obj.ymin) < 40:continue
        # if obj.occlusion < 0:continue
        # if obj.truncation > 0.15:continue
        # if obj.t[2] < 34.56:continue
        ...
        if obj.type=='Car':
            ...
            class_color = (218,112,214)
        
        cv2.rectangle(img1, (int(obj.xmin),int(obj.ymin)),
            (int(obj.xmax),int(obj.ymax)), class_color, 2)
        
        box3d_pts_2d, box3d_pts_3d = utils.compute_box_3d(obj, calib.P)
        img2 = utils.draw_projected_box3d(img2, box3d_pts_2d, class_color)
        i+=1
     # show 2D     
...
def show_image_with_boxes_PandL(img, objects, objects_L, calib, show3d=True):
    ''' Show image with 2D bounding boxes '''
    img1 = np.copy(img) # for 2d bbox
    img2 = np.copy(img) # for 3d bbox
...
def get_lidar_in_image_fov_lyft(pc_velo, calib, xmin, ymin, xmax, ymax,
                           return_more=False, clip_distance=4.0):
    ''' Filter lidar points, keep those in image FOV '''
    pts_2d = calib.project_velo_to_image(pc_velo)
...
def get_foreground_lidar_in_image_fov_kitti(pc_velo, calib, xmin, ymin, xmax, ymax,
                           return_more=False, clip_distance=2.0):
    ''' Filter lidar points, keep those in image FOV '''
    pts_2d = calib.project_rect_to_image(pc_velo)
    
    fov_inds = (pts_2d[:,0]<xmax) & (pts_2d[:,0]>=xmin) & \
        (pts_2d[:,1]<ymax) & (pts_2d[:,1]>=ymin)
    fov_inds = fov_inds & (pc_velo[:,2]>clip_distance)
    imgfov_pc_velo = pc_velo[fov_inds,:]
    if return_more:
        return imgfov_pc_velo , pts_2d, fov_inds
    else:
...
def get_foreground_lidar_in_image_fov_lyft(pc_velo, calib, xmin, ymin, xmax, ymax,
                           return_more=False, clip_distance=2.0):
    ''' Filter lidar points, keep those in image FOV '''
    pts_2d = calib.project_rect_to_image(pc_velo)
...
def show_lidar_with_boxes(pc_velo, objects, calib,
    ...
    for obj in objects:
        ...
        draw_gt_boxes3d([box3d_pts_3d_velo], fig=fig, color=class_color)
        # mlab.plot3d([x1, x2], [y1, y2], [z1,z2], color=(0.5,0.5,0.5),
        #     tube_radius=None, line_width=1, figure=fig)
    mlab.show(1)

def show_lidar_with_boxes_PandL(pc_velo, objects, objects_L, calib,
                          img_fov=False, img_width=None, img_height=None): 
    ''' Show all LiDAR points.
        Draw 3d box in LiDAR point cloud (in velo coord system) '''
    if 'mlab' not in sys.modules: import mayavi.mlab as mlab
    ...
    for obj in objects:
        ...
        if obj.type=='Car':
            ...
            class_color = (0.953125,0.484375,0.375)
        elif obj.type=='Cyclist':
            class_color = (0,1,1)
        # Draw 3d bounding box
        box3d_pts_2d, box3d_pts_3d = utils.compute_box_3d(obj, calib.P) 
    ...
    for obj in objects_L:
        ...
        if obj.type!='Car':continue
        # if (obj.type!='Pedestrian' and obj.type!='Cyclist'):continue
        ...
        if obj.type=='Car':
            ...
            class_color = (0.3671875,0.1484375,0.0703125)
        elif obj.type=='Cyclist':
            class_color = (0,0,1)
...
def show_lidar_on_image(pc_velo, img, calib, img_width, img_height):
    ...
    Image.fromarray(img).show() 
    return img, imgfov_pts_2d

def show_lidar_on_image_lyft(pc_velo, img, calib, img_width, img_height):
    ''' Project LiDAR points to image '''
    imgfov_pc_velo, pts_2d, fov_inds = get_lidar_in_image_fov_lyft(pc_velo,
        calib, 1, 1, img_width - 1, img_height - 1, True)
    imgfov_pts_2d = pts_2d[fov_inds,:]
    imgfov_pc_rect = calib.project_velo_to_rect(imgfov_pc_velo)
...
def show_foreground_lidar_on_image_kitti(pc_velo, img, calib, img_width, img_height):
    imgfov_pc_velo, pts_2d, fov_inds = get_foreground_lidar_in_image_fov_kitti(pc_velo,
        calib, 0, 0, img_width, img_height, True)
    imgfov_pts_2d = pts_2d[fov_inds,:]
    #imgfov_pc_rect = calib.project_velo_to_rect(imgfov_pc_velo)
...
def show_foreground_lidar_on_image_lyft(pc_velo, img, calib, img_width, img_height):
    imgfov_pc_velo, pts_2d, fov_inds = get_foreground_lidar_in_image_fov_lyft(pc_velo,
        calib, 0, 0, img_width, img_height, True)
    imgfov_pts_2d = pts_2d[fov_inds,:]
    #imgfov_pc_rect = calib.project_velo_to_rect(imgfov_pc_velo)

...
###################################################################################                        
                    ...
                    #     metrics["loc_loss_up3"] = float(
                    #         extra_loss["box_preds_up3"].detach().cpu().numpy())
                    #     metrics["cls_loss_up2"] = float(
                    #         extra_loss["cls_preds_up2"].detach().cpu().numpy())
                    #     metrics["cls_loss_up3"] = float(
                    #         extra_loss["cls_preds_up3"].detach().cpu().numpy())
                    # except:
                    #     metrics["loc_loss_up2"] = float(
                    #         extra_loss["box_preds_up2"])
                    #     metrics["loc_loss_up3"] = float(
                    #         extra_loss["box_preds_up3"])
                    #     metrics["cls_loss_up2"] = float(
                    #         extra_loss["cls_preds_up2"])
                    #     metrics["cls_loss_up3"] = float(
                    #         extra_loss["cls_preds_up3"])
###################################################################################
            ...
            for example in iter(eval_dataloader):
                example = example_convert_to_torch(example, float_dtype, GPU)
                if pickle_result:
...
def evaluate(config_path,
    ...
    if ckpt_path is None:
        ...
        if isinstance(net, nn.DataParallel):
            torchplus.train.restore(ckpt_path, net.module)
        else:
    ...
    eval_dataset = input_reader_builder.build(
        ...
        voxel_generator=voxel_generator,
        target_assigner=target_assigner)
    # print(eval_dataset[0]["num_points"])
    eval_dataloader = torch.utils.data.DataLoader(
        eval_dataset,
        batch_size=input_cfg.batch_size,
        shuffle=False,
    ...
    for example in iter(eval_dataloader):
        example = example_convert_to_torch(example, float_dtype, GPU)
        # print("============================================================================================================")
        # print("eval_data_index: ", eval_data_index)
        # print("eval_data: ", example)
        # print("============================================================================================================")
        if pickle_result:
            dt_annos += predict_kitti_to_anno(
                net, example, class_names, center_limit_range,

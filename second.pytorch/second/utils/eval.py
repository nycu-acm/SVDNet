...
def get_official_eval_result(gt_annos, dt_annos, current_classes, difficultys=[0, 1, 2], return_data=False):
    ...
    ##############################################   eval  different  distance GT PART ################################################
    # distance_flag = -1
    # for gt_anno_index in range(len(gt_annos)):
    #     temp_index = 0
        
    #     for gt_anno in gt_annos[gt_anno_index]["location"]:
       
    #         # if gt_anno[-1]>=20.0:
    #         #     gt_annos[gt_anno_index]["name"][temp_index] = "DontCare" 
    #         #     distance_flag = 0
            
    #         # if gt_anno[-1]<20.0 or gt_anno[-1]>=40.0:
    #         #     gt_annos[gt_anno_index]["name"][temp_index] = "DontCare"
    #         #     distance_flag = 1
    ...
    # #############################################   eval  different  distance DT PART ################################################
    # for dt_anno_index in range(len(dt_annos)):
    #     temp_index = 0
    #     for dt_anno in dt_annos[dt_anno_index]["location"]:
            
    #         if distance_flag == 0:          
    #             if dt_anno[-1]>=20.0:
    #                 dt_annos[dt_anno_index]["name"][temp_index] = "DontCare"  
            
    #         if distance_flag == 1:
    #             if dt_anno[-1]<20.0 or dt_anno[-1]>=40:
    #                 dt_annos[dt_anno_index]["name"][temp_index] = "DontCare" 
    ...
    #############################################   eval  select  confidence DT PART ################################################
    for dt_anno_index in range(len(dt_annos)):
        temp_index = 0
        for dt_anno in dt_annos[dt_anno_index]["score"]:
